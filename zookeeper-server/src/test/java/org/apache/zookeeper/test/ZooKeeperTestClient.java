...
public class ZooKeeperTestClient extends ZKTestCase implements Watcher {
    ...
    private void enode_test_1() throws IOException, InterruptedException, KeeperException {
        ...
        try {
            ...
            Code code = ke.code();
            boolean valid = code == KeeperException.Code.NODE_EXISTS;
            if (!valid) {
    ...
    private void enode_test_2() throws IOException, InterruptedException, KeeperException {
        ...
        if (stat_node != null) {
            ...
            try {
                ...
                Code code = ke.code();
                boolean valid = code == KeeperException.Code.NO_NODE || code == KeeperException.Code.NOT_EMPTY;
                if (!valid) {
        ...
        try {
            ...
            Code code = ke.code();
            boolean valid = code == KeeperException.Code.NOT_EMPTY;
            if (!valid) {
        ...
        try {
            ...
            Code code = ke.code();
            boolean valid = code == KeeperException.Code.NO_CHILDREN_FOR_EPHEMERALS;
            if (!valid) {
        ...
        try {
            ...
            Code code = ke.code();
            boolean valid = code == KeeperException.Code.NO_NODE;
            if (!valid) {
                fail("Unexpected exception code for createin: " + code);
            }
        }
    ...
    private void delete_create_get_set_test_1() throws IOException, InterruptedException, KeeperException {
        ...
        try {
            ...
            Code code = ke.code();
            boolean valid = code == KeeperException.Code.NOT_EMPTY || code == KeeperException.Code.BAD_VERSION;
            if (!valid) {
                fail("Unexpected exception code for delete: " + ke.getMessage());
            }
        }
